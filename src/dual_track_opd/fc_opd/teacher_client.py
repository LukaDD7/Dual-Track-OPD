...
class TeacherClient:
    ...
    def score(self, requests: Sequence[TeacherScoreRequest]) -> list[TeacherScoreResponse]:
        ...
        for request, response in zip(requests, responses, strict=True):
            ...
            if response.token_ids != request.response_token_ids:
                raise TeacherServiceError(
                    "teacher response token IDs do not match the exact student request"
                )
...
def _topk_from_response(response: TeacherScoreResponse) -> TeacherTopK:
    return TeacherTopK(
        ...
        valid_mask=torch.ones((1, len(response.token_ids)), dtype=torch.bool),
    )
