...
| 23 | **PTD-PO r3 首启数秒即崩（ModuleNotFoundError: narwhals，非 SIGKILL）** — 2026-09-05 13:59:42 启动 `qwen3vl_virl39k_ptd_base_4gpu_20260905_r3`，数秒内死于 import：`transformers→sklearn(1.9.0)→sklearn/utils/validation.py:14 import narwhals.stable.v2` 报 No module named 'narwhals'。日志仅 8.8KB 单个栈，Ray 未启动——与同日 0131/0736 的 raylet/gcs SIGKILL 死法完全无关。 | 训练 env `va-opd-qwen35-v090-cu132-r595-v1` 在 **13:04–14:10 有一个 pip 会话持续装 ~25 个包**（scikit_learn 13:45 升级 → narwhals 14:03 → threadpoolctl 14:10，另有 litellm/nltk/rouge/math_verify/matplotlib 等）。r3 恰在 13:59 撞进「sklearn 已升级、narwhals 还没装」的破窗——**被增量修改中的共享 env 存在不可 import 的中间态**。sklearn 1.9.0 新引入 narwhals 硬依赖，而 transformers 的 `AutoModel` import 链经 `generation/candidate_generator.py:31 from sklearn.metrics import roc_curve` 拉进 sklearn。 | 无需改代码：等 pip 结束直接重跑。Claude 14:22–14:32 全链验证（narwhals/sklearn/transformers/verl/ray 2.55.1/vllm 0.27.1/geo3k_reward 全 import OK，env 14:10 后无新写入）→ 同名重启即恢复。 | ✅ 已解（重跑即可） | 中。长期纪律：**训练 env 在起跑/运行期间禁止 pip/conda install**；需要新包用独立 env 或在训练启动前装完。同类前科见 environment_registry「mathruler 缺失 → reward 全失败 → batch 空崩溃」——都是「活 env 被动过」这一类事故。 |
| 24 | **盒上外部 SIGKILL 杀手点名 Ray 控制面（PTD-PO 0131/0736 全损）** — 2026-09-05 两个 run（`..._0131` step 35 / `..._0736` step 20）均死于 raylet + gcs_server 被 SIGKILL 级强杀、driver 挂起、全 run 零 checkpoint。签名：末条带时间戳日志后只剩无时间戳的 Ray event-stats 堆栈（2548 行）静默写入直至完全冻结；**无 SIGTERM 痕迹、无 OOM、无 Traceback**。发作周期 40–90 分钟，两次死亡 + 四次同型自愈 blip 同签名。 | 盒上存在周期性发作的外部杀手，精确点名 raylet+gcs_server。判别闭环（runbook 2026-09-05 根因判别结果表）：cgroup memory.events 全零（排除 OOM kill）、uptime 43 天（排除重启）、raylet/gcs 均为 `<defunct>` 而 main_ppo 挂起未死（非训练代码崩溃）、Ray 日志无崩溃栈（排除 Ray 自身 bug）。最大嫌疑 = **平台 per-进程资源上限（RSS/线程数）**，raylet/gcs 恰是进程树里 RSS 最高的两个常驻进程。dmesg 容器内无权限（无 CAP_SYSLOG），无法拿到内核 kill 记录做最终实锤；session `logs/old/` 为空（轮转目录无 raylet.err 尾巴），容器内取证已穷尽。注意：**与本表 #4/#6/#15 的历史死法不同**——那些是 NCCL allgather 死锁 SIGABRT（有超时栈），09-04 07:07 那次是显式 `ray stop`（有 SIGTERM 字样），本次无声 SIGKILL 是 2026-09-05 首现的新现象。 | 容器内无法根治（凶手在宿主/平台层）。缓解 = 修订路线 2：① save_freq=25 + `trainer.max_actor_ckpt_to_keep=2`（run_grpo_mmf_ptd.sh:159 已打补丁，峰值 ~196G 磁盘安全）；② 实验名固定同名重启（`resume_mode=auto` 自动从最新 ckpt 续训）；③ watchdog（cron 每分钟查 log mtime，冻结 >10 min → 杀 + 同名 nohup 重启）。根因级解决需平台确认 per-进程资源上限并调高，或换节点。 | 🔄 缓解中（watchdog + ckpt 保留） | 高。死亡窗口全损上限 = 25 步 ≈80 分钟；死点随机（0736 死在首个 ckpt 之前 = 全损先例）。若杀手持续发作，单次 run 可能需要多次 watchdog 自愈才能跑完 2397 步。 |
| 25 | **显式 ViRL 数据路径被 paper profile 覆盖回 Geometry3K** — 2026-09-19 新的 32B→8B/ViRL39K wrapper 已显式传 `--train-data/--val-data`，但 `run_va_opd_native.sh --profile paper` 无条件把两个路径改回 `geometry3k_va_paper`。第一次 preflight 显示 PASS 但 manifest 审计的是 2,101 行 Geometry3K，而非 38,348 行 ViRL39K。 | profile 默认值与显式 CLI 参数优先级倒置；与此前“显式模型路径被 paper profile 覆盖”是同一类 launcher bug。 | 增加 `TRAIN_DATA_EXPLICIT/VAL_DATA_EXPLICIT`，paper profile 只在没有显式路径时应用默认值；新增回归测试 `test_cli_data_paths_survive_paper_profile`。修正后全量 preflight manifest 确认 train dataset path/rows 为 `virl39k_va_opd/train.parquet` / 38,348。 | ✅ 已修 | 低。后续新增 profile 默认值时必须遵守“显式 CLI > 环境覆盖 > profile 默认”的优先级，并用 manifest 核对实际 dataset path/rows。 |
| 26 | **ViRL 4-step smoke 在 validation dataloader 失败：`image_offset 1 >= len(images) 1`** — 2026-09-19 `smoke4_v1` 完成 preflight 并加载 32B teacher/8B student 后，进入初始 validation 前死亡。raw parquet 本身占位符与图片数完全一致，但 `FCOPDDataset._clean_prompts` 使用 Geometry3K v1 builder 无条件在 question 前追加一个 `<image>`；ViRL question 原本已含 `<image>`，导致 1 图样本被清洗成 2 个占位符/1 张图。多图样本同样会被破坏。 | Prompt 合约最初是单图 Geometry3K 专用，未把 source question 中的占位符和行内图片数纳入重建逻辑。 | prompt builder 增加 `image_count`；清洗时先移除 question 中的原有 `<image>`，再按 `len(images)` 生成对应数量的占位符。新增单图防重复、多图数量匹配、非法 image_count 回归测试；真实 ViRL train 38,348 行和 val 500 行清洗后均为 0 mismatch，多图行 2,269/32 保持正确。 | ✅ 已修 | 低。后续新增数据源必须同时提供 `question`、`prompt`、`images`，并用“清洗后占位符数 == images 数”作为 preflight gate。 |
| 27 | **ViRL validation scorer 与数据答案契约不匹配** — 2026-09-19 审计发现，转换器保留 `yesno/has_number` 等 ViRL 答案，但 `smoke_reward.py` 只可靠支持 A–D MCQ 与纯数字。把 gold 原样放进 `\boxed{}` 时，train 8,113/38,348（21.1%）、val 99/500（19.8%）被判 0；`has_number` 和 `yesno` 100% 失败。虽然 VA-OPD 训练 loss 不使用 task reward，但 checkpoint 按 validation 曲线选择，因此该 bug 会系统性扭曲 checkpoint selection。 | scorer 将显式 final answer 强行压缩为首个字母/数字；boxed 解析不支持 nested LaTeX；gold 归一化误用 MCQ choice-value 前缀剥离；choices 为 numpy array 时 `or ()` 触发布尔歧义；浮点严格相等有二进制误差。 | 保留完整显式答案并做结构化归一；支持 A–E、yes/no/true/false、时间/比例/表达式/LaTeX/集合/坐标/单位；balanced-brace 解析支持 nested 与异常尾反斜杠；数值比较加容差；可选 mathruler 做数学等价。全量 ViRL gold-in-boxed contract：train 0/38,348 failure，val 0/500 failure。 | ✅ 已修 | 中。任何新数据源加入前必须运行同款 gold-in-boxed 全量审计；validation scorer 变更需与正式 eval repo 口径分开记录，避免把训练 monitor 当作外部 benchmark 结果。 |
| 28 | **ViRL scorer 后续审计发现 MCQ letter+value 回归与 mathruler 环境不可复现** — 2026-09-20 审计指出模型常输出 `A. 2`、`\text{(B) }30^\circ`、`C. \112^\circ`，此前修复只识别“纯单字母”，造成 18 条 MCQ 反向回归；同时 `mathruler` 是静默可选依赖，实际训练环境为 0.1.0 但未进 preflight/manifest，缺失时同一 validation 从 0.548 变 0.416。mathruler 对复合表达式/等式也存在宽松解析风险。 | MCQ 前缀解析过窄；`_math_equal` import 失败时静默返回 False；resolved environment 未记录 scorer 依赖；mathruler 被无条件用于所有非精确匹配。 | 1) MCQ 前缀必须显式形如 `A.`/`(B)`，避免把 `Angle E...` 误判为 A，同时支持 letter+value；2) cu128/cu132 constraints、preflight expected versions、实际 cu128 manifest 均锁定 `mathruler==0.1.0`，manifest 缺包 fail-fast；3) mathruler 只允许单一保守 LaTeX 表达式，排除 or/equation/inequality/range/单位文本/集合/坐标；4) 用 question+gold 双键映射重算 fcb807b 为 288/500=0.576、1 条旧对→新错；此前文档的 283/500 是 question-only 映射造成的 stale 口径。 | ✅ 已修 | 中。validation scorer 的依赖变化必须同步 constraints、preflight 和环境 manifest；不要把 mathruler 用于复合/文本答案。 |
| 29 | **v3 仍有 5 条真实输出低估与文档数字 stale** — 2026-09-20 后续审计发现 fcb807b 后 v3 final validation 仍有 5 条 old correct → new wrong：`\text{yes}/\text{no}` 未 unwrap；decimal `3.5` 被 math gate 意外拒绝；`y_{2}` vs `y_2` 冗余 braces 未归一；`72^\circ $.` 未清理 `$`；`y = ...` 与表达式 gold 未去掉左侧变量。文档还记录了旧的 v2 283/500，而 question+gold 双键映射下 fcb807b 实测为 288/500。 | yes/no 判断发生在 `_unwrap_latex_text` 之前；math gate 字符类漏掉小数点；canonical 未压缩 `_{}；boxed 输出可能带 LaTeX math delimiter 和左侧等式变量；旧审计映射只用 question，重复题样本会错误配对。 | yes/no 与 gold 均先 unwrap；math gate 允许 decimal 并拒绝变量后直接拼数字（防 `x0` 宽松误判）；canonical 移除 `$`、压缩 `y_{2}->y_2`；`y = expression` 在 gold 无等号时去掉左侧单变量；文档统一使用 question+gold 双键映射。修复后 v2 final=291/500=0.582（1 条 reverse，无 boxed 模糊长文本），v3 final=294/500=0.588（0 reverse）；gold-in-boxed train/val 0 failure，确定性负例 train/val 0 false positive。 | ✅ 已修 | 中。后续离线重算必须写明 question+gold 双键映射，且在修复 scorer 后重新发布数字，不能沿用旧口径。 |
| 30 | **Best-val protector 在 rolling best 被清理后误选低分新 checkpoint** — 2026-09-26 清理 rolling 625/750 后，历史 best 625 只存在于 protected 目录，但 protector 仍只把 rolling `global_step_*` 当作可用候选，误选了分数更低的 rolling 775，并替换 protected 625。 | `best_step()` 的 available 集合没有纳入 `protected_runs/global_step_*`；因此一旦 rolling best 被删，历史最高分会被视为不可用。 | `best_step()` 同时检查 rolling checkpoint 和 protected checkpoint；protected copy 完整时仍参与历史 best 竞争。新增回归测试模拟“protected 625 完整、rolling 625 已删、rolling 775 分数更低”，断言 marker 和 protected 目录都保持 625。 | ✅ 已修 | 中。清理 rolling best 前必须先运行修复后的 protector，并确认 marker source 指向 protected 目录；不要在 protector 未运行或未验证时删除历史 best 的唯一 rolling 副本。 |
| 31 | **Best-val protector 在 rolling best 变成 tombstone 时仍忽略 protected best** — 2026-09-26 step 800 达到 0.638 后，VERL retention 将 rolling 800 降为 129K tombstone；protector 的第一版修复只检查 `checkpoints.get(step, protected_path)`，由于 rolling 目录仍存在，它不会回退到 protected 800，于是误选低分 step 850 并删除 protected 800。 | Python `dict.get(default)` 只在 key 缺失时使用 default；rolling step 目录存在但不完整时，protected candidate 没有被检查。 | `best_step()` 对每个历史 score 同时构造 rolling candidate 和 protected candidate，只要任一完整即参与 best 竞争。新增回归测试模拟“protected 800 完整、rolling 800 只剩 tombstone、rolling 850 分数更低”，断言 marker 和 protected 目录都保持 800。 | ✅ 已修 | 高。VERL 的保留策略天然会留下 tombstone，未来所有 best 保护必须覆盖“rolling key 存在但不完整”的路径。 |
