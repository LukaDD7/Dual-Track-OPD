#!/usr/bin/env bash
# VA-OPD: Visual-Advantage On-Policy Distillation (arXiv 2605.21924)
# ===================================================================
# Pure distillation — no GRPO, no hidden-state gap, no verifier gates.
#
# Algorithm:
#   1. Student generates K=4 rollouts per prompt
#   2. Teacher scores under FULL and DEGRADED images
#   3. VA = log P_T(y|full) - log P_T(y|degraded)
#   4. Rollout weights = K * softmax(mean(top-20% VA) / tau)
#   5. Grouped KL: λ * mean(KL_HighVA) + (1-λ) * mean(KL_LowVA), λ=0.75
#   6. L_VA-OPD = Σ w^(k) * L_group^(k)  →  backward
#
# GPU layout:  0=Teacher(32B), 1..N-2=verl+vLLM, N-1=StudentScorer(4B)
#
# Usage:
#   bash scripts/hpc/run_verl_va_opd.sh [--steps S] [--gpus N] [--background]
...
if ${RUN_BACKGROUND}; then
    RELAUNCH_ARGS=()
    for arg in "$@"; do [[ "$arg" != "--background" ]] && RELAUNCH_ARGS+=("$arg"); done
    [[ -n "${PARQUET_OVERRIDE}" ]] && RELAUNCH_ARGS+=(--data "${PARQUET_OVERRIDE}")
    NOHUP_LOG="${REPO_ROOT}/artifacts/fc_opd/nohup_va_opd_$(date +%Y%m%d_%H%M%S).log"
    mkdir -p "$(dirname "${NOHUP_LOG}")"
    echo "Launching background VA-OPD training. Monitor: tail -f ${NOHUP_LOG}"
    nohup bash "$0" --gpus "${GPU_COUNT}" --steps "${NUM_STEPS}" "${RELAUNCH_ARGS[@]}" > "${NOHUP_LOG}" 2>&1 &
    disown; echo "Background PID: $!"; exit 0
fi
...
CHECKPOINT_DIR="${REPO_ROOT_ABS}/checkpoints/verl_va_opd/${RUN_ID}"

echo "══════════════════════════════════════════════════════════════"
echo "  VA-OPD Training — arXiv 2605.21924"
echo "  Run ID:       ${RUN_ID}"
echo "  Steps:        ${NUM_STEPS}"
echo "  Loss mode:    va_opd (pure distillation, λ=0.75)"
echo "  Rollout n:    ${ROLLOUT_N}  (K=4)"
...
# ── verify ──────────────────────────────────────────────────────────────────
if ! grep -q "verl_actor_loss" third_party/verl/verl/workers/actor/dp_actor.py; then
    echo "FATAL: VA-OPD actor patch not applied"; exit 1
fi
...
for i in $(seq 1 300); do
    if curl -s "http://127.0.0.1:${TEACHER_PORT}/health" >/dev/null 2>&1; then break; fi
    if ! kill -0 ${TEACHER_PID} 2>/dev/null; then echo "TEACHER DIED"; tail -20 "${TEACHER_LOG}"; exit 1; fi
    sleep 1
done
...
SCORER_PID=$!
for i in $(seq 1 300); do
    if curl -s "http://127.0.0.1:${SCORER_PORT}/health" >/dev/null 2>&1; then break; fi
    if ! kill -0 ${SCORER_PID} 2>/dev/null; then echo "SCORER DIED"; tail -20 "${SCORER_LOG}"; exit 1; fi
    sleep 1
done
