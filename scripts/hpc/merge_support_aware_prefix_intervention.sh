#!/usr/bin/env bash
# Strictly merge completed frozen prefix-intervention shards.
...
for (( shard_index=0; shard_index<NUM_SHARDS; shard_index++ )); do
    ...
    [[ -f "${shard_dir}/run_manifest.json" ]] || { echo "FATAL: missing ${shard_dir}/run_manifest.json" >&2; exit 1; }
    SHARD_DIRS+=("${shard_dir}")
done

exec "${PYTHON_BIN}" -u -m dual_track_opd.support_aware.prefix_intervention merge \
